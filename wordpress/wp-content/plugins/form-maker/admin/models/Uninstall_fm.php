...
class FMModelUninstall_fm {
  public function delete_db_tables() {
    global $wpdb;
    $true_or_false_forms = WDFM()->is_pro ? false : $wpdb->get_var("SELECT COUNT(*) FROM " . $wpdb->prefix . "formmaker WHERE `id` IN (" . (get_option('contact_form_forms', '') != '' ? get_option('contact_form_forms') : 0) . ")");
    if ($true_or_false_forms) {
      ...
      $frontend_js = WDFM()->plugin_dir . '/js/frontend/';
      ...
      $frontend_css = WDFM()->plugin_dir . '/css/frontend/';
