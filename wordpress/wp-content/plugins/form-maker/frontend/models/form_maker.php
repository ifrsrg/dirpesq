...
class FMModelForm_maker {
  ...
  public function showform( $id, $type = 'embedded' ) {
    global $wpdb;
    $row = $wpdb->get_row($wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'formmaker WHERE id="%d"' . (WDFM()->is_pro ? '' : ' AND id NOT IN (' . (get_option('contact_form_forms', '') != '' ? get_option('contact_form_forms') : 0) . ')'), $id));

    if ( !$row ) {
      echo WDW_FM_Library::message(__('Something went wrong when trying to display this form.', WDFM()->prefix), 'error');
      return FALSE;
    }

    if ( $row->type != $type ) {
      echo WDW_FM_Library::message(__('The form you are trying to view does not have Embedded display type.', WDFM()->prefix), 'error');
      return FALSE;
    }
    ...
    if ( !$form_preview && !$row->published ) {
      ...
      echo WDW_FM_Library::message(__('The form you are trying to view has been unpublished.', WDFM()->prefix), 'error');
      return FALSE;
    }
    ...
    wp_register_style('fm-style-' . $theme_id, WDFM()->plugin_url . '/css/frontend/fm-style-' . $theme_id . '.css', array(), $cssver);
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form .fm-header{
	...
	color:" . $form_theme['HDPColor'] . ";
      ...
      $css_content .= ".fm-form-container.fm-theme" . $theme_id . " .fm-form-message {
	...
	text-align: center;
      ...
      $css_content .= ".fm-form-container.fm-theme" . $theme_id . " .fm-success.fm-form-message {
...
.fm-form-container.fm-theme" . $theme_id . " .minimize-form {
	top: " . $form_theme['CBPTop'] . ";
	" . $for_mini . ": " . (2 * (int)$form_theme['CBP' . ucfirst($for_mini)] + $form_theme['CBPFontSize'] + 3) . "px;
	bottom: " . $form_theme['CBPBottom'] . ";
}";
      if ( $borders['CBP'] ) {
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form .file-picker{
	background: url(" . WDFM()->plugin_url . '/' . $form_theme['OPFBgUrl'] . ") " . $form_theme['OPFBGRepeat'] . " " . $form_theme['OPFPos1'] . " " . $form_theme['OPFPos2'] . ";
	display: inline-block;
}

.fm-form-container.fm-theme" . $theme_id . " .fm-form .wdform-calendar-button {
	background: url(" . WDFM()->plugin_url . '/' . $form_theme['OPDPIcon'] . ") " . $form_theme['OPDPRepeat'] . " " . $form_theme['OPDPPos1'] . " " . $form_theme['OPDPPos2'] . "; 
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form select {
	...
	background:" . $form_theme['IPBGColor'] . " url(" . WDFM()->plugin_url . '/' . $form_theme['SBPBackground'] . ") " . $form_theme['SBPBGRepeat'] . " " . $form_theme['SBPBGPos1'] . " " . $form_theme['SBPBGPos2'] . ";
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form .checkbox-div input[type='checkbox']:checked + label span:after {
	...
	background:" . ($form_theme['MCCPBackground'] ? ($form_theme['MCCPBGColor'] . " url(" . WDFM()->plugin_url . '/' . $form_theme['MCCPBackground'] . ") " . $form_theme['MCCPBGRepeat'] . " " . $form_theme['MCCPBGPos1'] . " " . $form_theme['MCCPBGPos2']) : $form_theme['MCCPBGColor']) . ";
...
.fm-form-container.fm-theme" . $theme_id . " .fm-form .next-page div.wdform-page-button:hover {
	...
	color:" . $form_theme['NBHPColor'] . " !important;
      ...
      $defaultStyles .= ".fm-form-container.fm-theme" . $theme_id . " .fm-minimize-text div{
