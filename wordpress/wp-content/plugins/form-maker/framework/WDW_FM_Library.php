...
class WDW_FM_Library {
    ...
    if ($message_id) {
      switch ( $message_id ) {
        ...
        case 10:
          ...
          break;
        case 11:
          $message = 'Item succesfully duplicated.';
          $type = 'updated';
          break;
		case 12:
          $message = 'IP Succesfully Blocked.';
          $type = 'updated';
		break;	
		case 13:
          $message = 'IP Succesfully Unblocked.';
          $type = 'updated';
		break;	
		case 14:
  ...
  public static function message($message, $type) {
    if ( $message ) {
      ob_start();
      ?><div style="width: 100%;" class="fm-form-message <?php echo $type; ?>">
      <div>
        <strong><?php echo $message; ?></strong>
      </div>
      </div><?php
  ...
  public static function create_js( $form_id, $force_rewrite = FALSE ) {
    $frontend_js = WDFM()->plugin_dir . '/js/frontend/fm-script-' . $form_id . '.js';
  ...
  jQuery("#wdform_' . $id1 . '_day' . $form_id . '").blur(function() {if (jQuery(this).val() == "0") jQuery(this).val(""); else add_0(this)});';
              $onload_js .= '
  jQuery("#wdform_' . $id1 . '_day' . $form_id . '").keypress(function() {return check_day(event, this)});';
            }
  ...
  jQuery("#wdform_' . $id1 . '_month' . $form_id . '").blur(function() {if (jQuery(this).val() == "0") jQuery(this).val(""); else add_0(this)});';
              $onload_js .= '
  jQuery("#wdform_' . $id1 . '_month' . $form_id . '").keypress(function() {return check_month(event, this)});';
            }
            if ( $param['w_year_type'] != "SELECT" ) {
              $onload_js .= '
  jQuery("#wdform_' . $id1 . '_year' . $form_id . '").keypress(function() {return check_year1(event, this)});';
              $onload_js .= '
  jQuery("#wdform_' . $id1 . '_year' . $form_id . '").change(function() {change_year(this)});';
            }
            if ( $required ) {
              array_push($req_fields, $id1);
            }
