...
function fm_delete_ip( id ) {
  ...
  post_data["nonce_fm"] = jQuery("#nonce_fm").val();
  post_data["task"] = "delete_blocked_ip";

  jQuery.post(jQuery("#blocked_ips").attr("action"), post_data, function (data) {
    }
  ).success(function (data, textStatus, errorThrown) {

    var paged = parseInt( jQuery(data).find("#total_for_paging").html() );
    ...
    jQuery('.wrap').load(url + ' #blocked_ips',function(){
      ...
      if (jQuery(".updated").length != 0) {
        jQuery(" .updated p strong").html("Items Succesfully Deleted");
      } else {
        jQuery("<div class='updated below-h2'><p><strong>Items Succesfully Deleted.</strong></p></div>").insertBefore("#blocked_ips");
      }
      // Set no items row width.
      set_no_items();
    }); //reload the form
...
function fm_save_ip( id ) {
  ...
  post_data["ip"] = ip;
  post_data["nonce_fm"] = jQuery("#nonce_fm").val();
  post_data["task"] = "insert_blocked_ip";
  ...
  jQuery.post(jQuery("#blocked_ips").attr("action"), post_data, function ( data ) {
    ...
    jQuery("#fm_blocked_ips_message").html("<div class='updated'><strong><p>Items Succesfully Inserted.</p></strong></div>");
...
function fm_insert_blocked_ip() {
  ...
  jQuery.post( jQuery("#blocked_ips").attr("action"), post_data, function (data) {
    ...
    jQuery('.wrap').load(url + ' #blocked_ips',function() {
      window.history.pushState(null, null, url);
      if (jQuery(".updated").length != 0) {
        jQuery(".updated p strong").html("Items Succesfully Saved");

      } else {
        jQuery("<div class='updated below-h2'><p><strong>Items Succesfully Saved.</strong></p></div>").insertBefore("#blocked_ips");
      }
    });//reload the form
  });
}
...
function fm_update_blocked_ip(id) {
  ...
  post_data["current_id"] = id;
  post_data["nonce_fm"] = jQuery("#nonce_fm").val();
  post_data["task"] = "update_blocked_ip";
  ...
  jQuery.post(url, post_data, function (data) {
    ...
    jQuery('.wrap').load(url + ' #blocked_ips',function() {
      if (jQuery(".updated").length != 0) {
        window.history.pushState(null, null, url);
        jQuery(".updated p strong").html("Items Succesfully Updated");

      } else {
        jQuery("<div class='updated below-h2'><p><strong>Items Succesfully Updated.</strong></p></div>").insertBefore("#blocked_ips");
      }
    });//reload the form
  });
}
