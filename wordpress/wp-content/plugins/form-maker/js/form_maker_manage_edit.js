...
function FormManageSubmitButton(check_for_changes) {
  ...
  if (check_for_changes) {
    ...
    if (jQuery('#header_description_initial_value').val() != jQuery('#header_description').val()) {
      form_changed = true;
    }
...
function formOnload(rows) {
	...
	for (i = 1; i <= 30; i++) {
		if (document.getElementById('form_id_tempform_view' + i)) {
			...
			document.getElementById('form_id_tempform_view' + i).parentNode.appendChild(page_toolbar_wrap);							
		}
	}
	...
	if (document.getElementById('form_id_tempform_view_img1') && form_view_count == 1) {
		document.getElementById("form_id_tempform_view_img1").setAttribute("class","form_view_hide");
	}
}
